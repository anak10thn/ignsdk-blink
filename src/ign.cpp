...
void ign::init(){
    ...
    //web->settings()->setAttribute(QWebEngineSettings::OfflineStorageDatabaseEnabled, true);
    //web->settings()->setAttribute(QWebEngineSettings::OfflineWebApplicationCacheEnabled, true);
    ...
    //web->settings()->setAttribute(QWebEngineSettings::JavaEnabled,true);
    //web->settings()->setAttribute(QWebEngineSettings::AcceleratedCompositingEnabled,true);
    ...
    //config mode disable
    web->page()->action(QWebEnginePage::Back)->setVisible(false);
    web->page()->action(QWebEnginePage::Forward)->setVisible(false);
    web->page()->action(QWebEnginePage::Reload)->setVisible(false);
    web->page()->action(QWebEnginePage::Stop)->setVisible(false);
}
...
void ign::back(){
    this->web->page()->triggerAction(QWebEnginePage::Back,true);
}

void ign::forward(){
    this->web->page()->triggerAction(QWebEnginePage::Forward,true);
}

void ign::stop(){
    this->web->page()->triggerAction(QWebEnginePage::Stop,true);
}

void ign::reload(){
    this->web->page()->triggerAction(QWebEnginePage::Reload,true);
}

void ign::cut(){
    this->web->page()->triggerAction(QWebEnginePage::Cut,true);
}

void ign::copy(){
    this->web->page()->triggerAction(QWebEnginePage::Copy,true);
}

void ign::paste(){
    this->web->page()->triggerAction(QWebEnginePage::Paste,true);
}

void ign::undo(){
    this->web->page()->triggerAction(QWebEnginePage::Undo,true);
}

void ign::redo(){
    this->web->page()->triggerAction(QWebEnginePage::Redo,true);
}

/* debuging mode */
void ign::setDev(bool v){
    //this->frame->settings()->setAttribute(QWebEngineSettings::DeveloperExtrasEnabled, v);
    this->debugging = true;
}
...
void ign::setDevRemote(int port){
    ...
    qputenv("QTWEBENGINE_REMOTE_DEBUGGING", server.toUtf8());
    //this->web->page()->setProperty("_q_webInspectorServerPort",port);
}

/* web security */
void ign::websecurity(bool c){
    web->settings()->setAttribute(QWebEngineSettings::LocalContentCanAccessRemoteUrls,c);
}
/* window config */
    ...
    if(config_file.open(QIODevice::ReadOnly)){
        ...
        foreach (QVariant button, result["button"].toList()) {
          ...
          if (button.toString() == "back"){
              web->page()->action(QWebEnginePage::Back)->setVisible(true);
          }
          if (button.toString() == "forward"){
              web->page()->action(QWebEnginePage::Forward)->setVisible(true);
          }
          if (button.toString() == "stop"){
              web->page()->action(QWebEnginePage::Stop)->setVisible(true);
          }
          if (button.toString() == "reload"){
              web->page()->action(QWebEnginePage::Reload)->setVisible(true);
          }
